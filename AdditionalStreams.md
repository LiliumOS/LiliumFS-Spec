...
* `SipHashC-D` - The SipHash algorithm, with C compression and D finalization rounds, where C and D are written using ascii decimal digits, and a 64-bit result, using the first 2 elements of the `seed` array. At the very least, C=2 and D=4, as well as C=1 and D=3 should be supported.
